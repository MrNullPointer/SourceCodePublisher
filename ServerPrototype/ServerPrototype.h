...
* ----------------------
*  ver1.3 : 30 Apr 2019
*  - added getPublish function to process message which contains cmd "get publish"
*  ver 1.2 : 22 Apr 2018
...
namespace ServerRepository
{
  ...
  public:
    ...
    std::string setSendFilePath(const std::string& path);
    std::string setSaveFilePath(const std::string& path);
  ...
  inline std::string Server::setSendFilePath(const std::string& path)
  {
    comm_.setSendFilePath(path);
  }
  inline std::string Server::setSaveFilePath(const std::string& path)
  {
